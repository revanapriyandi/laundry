<x-app-layout>
    ...
    <x-link-header type="button" x-data="" x-on:click.prevent="$dispatch('open-modal', 'tambah-pesanan')">
        Tambah Layanan
    </x-link-header>
            ...
            <div
                ...
                <div class="border-black/12.5 rounded-t-2xl border-b-0 border-solid p-4 pb-0">
                    <div class="flex items-center justify-between">
                        ...
                        <form method="post" action="{{ route('pesanan.updateStatus', $data->id) }}">
                            @csrf
                            <button type="submit"
                                class="inline-block px-6 py-3 mb-0 ml-auto font-bold text-center text-white uppercase align-middle transition-all border-0 rounded-lg cursor-pointer hover:scale-102 active:opacity-85 hover:shadow-soft-xs bg-gradient-to-tl from-gray-600 to-slate-300 leading-pro text-xs ease-soft-in tracking-tight-soft shadow-soft-md bg-150 bg-x-25">Pesan
                                </>
                        </form>
                    </div>
                </div>
